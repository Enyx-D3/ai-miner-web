...
execFileSync("tsc",["-p","tsconfig.v7-fixtures.json","--pretty","false"],{stdio:"inherit"});
const require=createRequire(import.meta.url);
const {normalizeArchiveConversation}=require("../.v7-test-dist/archiveNormalize.js");
const {canonicalMessageId}=require("../.v7-test-dist/identity.js");
const {atomizeMessage}=require("../.v7-test-dist/atomizer.js");
const {reconcileAtomToTruth}=require("../.v7-test-dist/truthEngine.js");
const {buildPatterns}=require("../.v7-test-dist/patternEngine.js");
const {evaluatePattern,createPatternTest,createPortableExpertise}=require("../.v7-test-dist/patternLab.js");
const {planRetrieval}=require("../.v7-test-dist/retrievalRouter.js");
const {compileB2Job,verifyB2Result}=require("../.v7-test-dist/jobs.js");
const {buildReasoningRequest,validateReasoningResult}=require("../.v7-test-dist/reasoning.js");
...
// V7 scale/UX invariants: bounded hot boot, persistent derived index, lazy full-history screens, and full-state B2M export.
const storeSource=fs.readFileSync("src/lib/brain2/store.ts","utf8");
const bootSource=storeSource.slice(storeSource.indexOf("export async function bootBrain2"),storeSource.indexOf("async function buildMutation"));
assert(!bootSource.includes('all<MessageRecord>("messages")')&&!bootSource.includes('all<AtomRecord>("atoms")'),"V7 boot loads full message/atom corpus");
assert(storeSource.includes("HOT_MESSAGE_LIMIT = 600")&&storeSource.includes("HOT_ATOM_LIMIT = 1200"),"bounded V7 hot set missing");
assert(storeSource.includes('createIndex("byTerm"')||storeSource.includes('ensureIndex(searchDocs,"byTerm"'),"persistent term index missing");
assert(storeSource.includes('derivedExcluded:["searchDocs"]'),"B2M does not exclude/rebuild derived search index");
const uiSource=fs.readFileSync("src/features/brain2/Brain2Workspace.tsx","utf8");
for(const needle of ["loadConversationMessages","loadProjectAtoms","loadProjectTruthSummary","searchBrain2Async","verifyB2ResultAsync","Memory Health","Repair search"])assert(uiSource.includes(needle),`V7 lazy UX missing ${needle}`);
console.log("V7 regression PASS: V5 invariants + relation-safe structured atomization, Pattern Lab explicit promotion, Portable Expertise, B/F/G retrieval routing, hashed Databox provenance, reasoning proposal-only contract, durable >2K encrypted queue.");
