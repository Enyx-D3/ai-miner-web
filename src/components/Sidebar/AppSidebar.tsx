...
export function AppSidebar(props: React.ComponentProps<typeof Sidebar>) {
  ...
  return <Sidebar {...props} className="border-r border-border bg-white/90">
    <SidebarHeader className="border-b border-border p-4"><Link href="/" className="flex items-center gap-2.5 transition-transform hover:opacity-90 active:scale-95"><span className="flex size-9 items-center justify-center rounded-xl bg-gradient-to-br from-cyan-400 via-blue-600 to-violet-600 font-tight text-xs font-black text-white shadow-[0_8px_22px_rgba(37,99,235,.2)]">B2</span><span className="leading-none"><span className="block font-tight text-xs font-extrabold tracking-[-.02em]">Brain2 Labs</span><span className="mt-1 block text-[9px] font-semibold text-muted-foreground">AI Miner · Mission Control</span></span><span className="ml-auto flex items-center gap-1 rounded-full border border-emerald-200 bg-emerald-50 px-2 py-1 text-[8px] font-black text-emerald-700"><Radio className="size-2.5"/> LOCAL</span></Link></SidebarHeader>
