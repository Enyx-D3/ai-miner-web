...
const MOBILE_SYNC_TABLE_ALIASES: Record<string, SyncTableName> = {
  capabilities: "compiledCapabilities",
  failureMemory: "failureMemories",
};
function canonicalIncomingSyncTable(table:string): SyncTableName | null {
  const canonical = MOBILE_SYNC_TABLE_ALIASES[table] ?? table;
  return SYNC_TABLE_SET.has(canonical) ? (canonical as SyncTableName) : null;
}
...
type Brain2ExactSearchKind = "message" | "atom" | "truth";
type Brain2ExactSearchOptions = { projectId?: string; cursor?: string; limit?: number; kinds?: Brain2ExactSearchKind[] };
type Brain2ExactSearchHit = {
  type: Brain2ExactSearchKind;
  recordId: string;
  text: string;
  score: number;
  matchedTerms: string[];
...
const BRAIN2_SEARCH_STOPWORDS = new Set(["the","and","for","with","that","this","from","into","your","our","what","when","where","which","who","why","how","are","was","were","been","being","have","has","had","will","would","should","could","can","may","might","must","about","after","before","then","than","them","they","their","there","here","also","just","only","very","more","most","much","many","some","any","all","not","but","you","please","is","of","we","do","to","a","an","in","on"]);
function brain2SearchTokens(value:string):string[]{const raw=String(value??"").toLowerCase().match(/[a-z0-9]+(?:[._-][a-z0-9]+)*/g)??[];const out:string[]=[];const seen=new Set<string>();for(const token of raw){const t=token.replace(/^[._-]+|[._-]+$/g,"");if(!t||BRAIN2_SEARCH_STOPWORDS.has(t)||seen.has(t))continue;seen.add(t);out.push(t);}return out;}
function brain2NormalizedPhrase(value:string):string{return brain2SearchTokens(value).join(" ");}
function brain2QueryHash(query:string,projectId:string,kinds:Brain2ExactSearchKind[]):string{const input=`${query}\u241f${projectId}\u241f${kinds.join(",")}`;let hash=2166136261;for(let i=0;i<input.length;i++){hash^=input.charCodeAt(i);hash=Math.imul(hash,16777619);}return (hash>>>0).toString(16).padStart(8,"0");}
function brain2ProjectName(projectId?:string):string|undefined{if(!projectId)return undefined;const project=snapshot.projects.find((item)=>item.id===projectId);return project?.name??project?.slug??project?.id;}
function brain2SearchDocProjectId(doc:PersistentSearchDocument):string|undefined{if(doc.projectId)return doc.projectId;if(!doc.conversationId)return undefined;return snapshot.conversations.find((c)=>c.id===doc.conversationId)?.projectId;}
function brain2ScoreSearchDoc(doc:PersistentSearchDocument,query:string,queryTokens:string[],projectId?:string):Brain2ExactSearchHit|null{const docProjectId=brain2SearchDocProjectId(doc);if(projectId&&docProjectId!==projectId)return null;const matchAll=query.trim()==="*";const projectName=brain2ProjectName(docProjectId);const text=String(doc.text??"");const textTokens=new Set(brain2SearchTokens(text));const termTokens=new Set((Array.isArray(doc.terms)?doc.terms:[]).flatMap((term)=>brain2SearchTokens(String(term))));const projectTokens=new Set(brain2SearchTokens(projectName??""));const matchedTerms=queryTokens.filter((term)=>textTokens.has(term)||termTokens.has(term)||projectTokens.has(term));if(!matchAll&&!matchedTerms.length)return null;let score=matchAll?1:0;if(!matchAll){const phrase=brain2NormalizedPhrase(query);const textPhrase=brain2NormalizedPhrase(text);const projectPhrase=brain2NormalizedPhrase(projectName??"");if(phrase&&textPhrase.includes(phrase))score+=100;if(phrase&&projectPhrase.includes(phrase))score+=80;for(const term of matchedTerms){if(textTokens.has(term))score+=25;if(termTokens.has(term))score+=15;if(projectTokens.has(term))score+=20;}if(queryTokens.length&&matchedTerms.length===queryTokens.length)score+=50;score+=Math.round((matchedTerms.length/Math.max(1,queryTokens.length))*20);}return{type:doc.kind,recordId:doc.recordId,text,score,matchedTerms,matchCoverage:matchAll?1:matchedTerms.length/Math.max(1,queryTokens.length),projectId:docProjectId,projectName,conversationId:doc.conversationId,messageId:doc.messageId,sourceId:doc.sourceId,createdAt:doc.createdAt,status:doc.status,role:doc.role,atomKind:doc.atomKind,confidence:doc.confidence};}
function brain2ParseSearchCursor(cursor:string|undefined,expectedHash:string,expectedSnapshotVersion:number):number{if(!cursor)return 0;const [version,snapshotPart,offsetPart,hashPart]=cursor.split(":");if(version!=="b2s1"||hashPart!==expectedHash)throw new Error("Invalid Brain2 search cursor.");const cursorSnapshot=Number(snapshotPart);const offset=Number(offsetPart);if(!Number.isInteger(cursorSnapshot)||!Number.isInteger(offset)||offset<0)throw new Error("Invalid Brain2 search cursor.");if(cursorSnapshot!==expectedSnapshotVersion)throw new Error("Brain2 search cursor is stale because AI Miner changed. Restart the search from the first page.");return offset;}
export async function getBrain2SearchInventory(){await bootBrain2();const [indexedDocuments,totalProjects,totalSources,totalConversations]=await Promise.all([countTable("searchDocs"),countTable("projects"),countTable("sources"),countTable("conversations")]);const expectedDocuments=(snapshot.storage?.totalMessages??0)+(snapshot.storage?.totalAtoms??0)+(snapshot.storage?.totalTruths??0);return{memoryRoot:snapshot.memoryRoot,snapshotVersion:snapshot.version,totals:{messages:snapshot.storage?.totalMessages??0,atoms:snapshot.storage?.totalAtoms??0,truths:snapshot.storage?.totalTruths??0,conversations:totalConversations,projects:totalProjects,sources:totalSources,indexedDocuments,expectedSearchDocuments:expectedDocuments},searchIndex:{status:snapshot.storage?.retrievalIndexStatus??"UNKNOWN",indexedDocuments,expectedDocuments,coverageRatio:expectedDocuments?Math.min(1,indexedDocuments/expectedDocuments):1,complete:indexedDocuments===expectedDocuments}};}
export async function searchBrain2ExactPage(query:string,options:Brain2ExactSearchOptions={}){await bootBrain2();const cleanQuery=String(query??"").trim();if(!cleanQuery)throw new Error("Missing Brain2 search query.");const limit=Math.max(1,Math.min(200,Number(options.limit??100)));const kinds=(options.kinds?.length?options.kinds:["message","atom","truth"]) as Brain2ExactSearchKind[];const projectId=String(options.projectId??"");const queryTokens=cleanQuery==="*"?[]:brain2SearchTokens(cleanQuery);if(cleanQuery!=="*"&&!queryTokens.length)throw new Error("Search query has no meaningful terms.");const queryHash=brain2QueryHash(cleanQuery,projectId,kinds);const offset=brain2ParseSearchCursor(options.cursor,queryHash,snapshot.version);const docs=await all<PersistentSearchDocument>("searchDocs");const allowedKinds=new Set(kinds);const matches:Brain2ExactSearchHit[]=[];for(const doc of docs){if(!allowedKinds.has(doc.kind))continue;const hit=brain2ScoreSearchDoc(doc,cleanQuery,queryTokens,projectId||undefined);if(hit)matches.push(hit);}matches.sort((a,b)=>b.score-a.score||String(b.createdAt??"").localeCompare(String(a.createdAt??""))||a.type.localeCompare(b.type)||a.recordId.localeCompare(b.recordId));const totalMatches=matches.length;const page=matches.slice(offset,offset+limit);const nextOffset=offset+page.length;const exhausted=nextOffset>=totalMatches;const inventory=await getBrain2SearchInventory();return{query:cleanQuery,queryTokens,projectId:projectId||undefined,snapshotVersion:snapshot.version,memoryRoot:snapshot.memoryRoot,offset,limit,totalMatches,results:page,nextCursor:exhausted?null:`b2s1:${snapshot.version}:${nextOffset}:${queryHash}`,exhausted,coverage:{searchIndexComplete:inventory.searchIndex.complete,searchIndexStatus:inventory.searchIndex.status,indexedDocuments:inventory.searchIndex.indexedDocuments,expectedDocuments:inventory.searchIndex.expectedDocuments,cursorExhausted:exhausted,exhaustiveForIndexedCorpus:exhausted&&inventory.searchIndex.complete}};}
...
export async function getSyncBootstrapManifest(){await bootBrain2();const tableCounts:Record<string,number>={};for(const table of [...SYNC_TABLES,"mutations" as const])tableCounts[table]=await countTable(table);const totalRecords=Object.values(tableCounts).reduce((sum,count)=>sum+count,0);return{memoryRoot:snapshot.memoryRoot,totalRecords,tableCounts};}
...
export async function removeSyncPeer(peerDeviceId:string){await bootBrain2();const id=`peer_${peerDeviceId}`;const db=await openDb();await new Promise<void>((resolve,reject)=>{const tx=db.transaction("syncPeers","readwrite");tx.objectStore("syncPeers").delete(id);tx.oncomplete=()=>resolve();tx.onerror=()=>reject(tx.error);tx.onabort=()=>reject(tx.error??new Error("Sync peer removal aborted"));});snapshot.syncPeers=snapshot.syncPeers.filter((peer)=>peer.id!==id&&peer.peerDeviceId!==peerDeviceId);setSnapshot({syncPeers:snapshot.syncPeers});}
...
export async function markPeerAck(peerDeviceId:string,sequence:number){const peer=await upsertSyncPeerInternal(peerDeviceId);await upsertSyncPeerInternal(peerDeviceId,{lastPushedSequence:Math.max(peer.lastPushedSequence,sequence),lastSyncAt:now(),status:"SYNCING",error:undefined});}
...
export async function applyReplicatedMutations(peerDeviceId:string,incoming:MutationRecord[],transport:"WEBRTC"|"HTTPS"|"B2_NETWORK"="WEBRTC"){
  ...
  await upsertSyncPeerInternal(peerDeviceId,{lastAppliedPeerSequence:lastApplied,lastSyncAt:now(),lastSeenAt:now(),status:rejected.length?"CONFLICT":"SYNCING",transport,error:rejected.length?rejected[0].reason:undefined});if(accepted.length){const state=await refreshStorageState();rebuildIngestionIndexes();setSnapshot({sources:snapshot.sources,conversations:snapshot.conversations,messages:snapshot.messages,atoms:snapshot.atoms,truths:snapshot.truths,projects:snapshot.projects,ticks:snapshot.ticks,decisions:snapshot.decisions,patterns:snapshot.patterns,experiments:snapshot.experiments,missions:snapshot.missions,checkpoints:snapshot.checkpoints,verifications:snapshot.verifications,transactions:snapshot.transactions,patternTests:snapshot.patternTests,portableExpertise:snapshot.portableExpertise,compiledCapabilities:snapshot.compiledCapabilities,reasoningTrajectories:snapshot.reasoningTrajectories,failureMemories:snapshot.failureMemories,databoxes:snapshot.databoxes,evidenceBlocks:snapshot.evidenceBlocks,mutations:snapshot.mutations,syncPeers:snapshot.syncPeers,storage:state});const deltaPayloads=sorted.map((m)=>m.payload).filter((payload):payload is MutationDeltaPayload=>Boolean(payload));if(deltaPayloads.some((payload)=>Boolean(payload.writes.atoms?.length||payload.writes.truths?.length))){scheduleDerivedPatternsRefresh(collectProjectIdsFromDeltaPayloads(deltaPayloads));}}return{accepted,rejected,lastAppliedSequence:lastApplied};
}

function syncMergeClock(record:AnyRecord){for(const key of ["updatedAt","createdAt","occurredAt","timestamp","lastSeenAt","completedAt"]){const value=String(record[key]??"").trim();if(value)return value;}return "";}
async function syncMergeWinner(local:AnyRecord,incoming:AnyRecord){const localJson=canonicalJson(local),incomingJson=canonicalJson(incoming);if(localJson===incomingJson)return local;const lc=syncMergeClock(local),ic=syncMergeClock(incoming);if(lc!==ic)return ic.localeCompare(lc)>0?incoming:local;const [lh,ih]=await Promise.all([sha256(localJson),sha256(incomingJson)]);return ih.localeCompare(lh)>0?incoming:local;}
export async function applySyncMergeChunk(targetRoot:string,table:string,records:AnyRecord[]){await bootBrain2();if(snapshot.memoryRoot!==targetRoot)throw new Error("Merge target memory-root mismatch.");const canonical=canonicalIncomingSyncTable(table);if(!canonical)throw new Error(`Unsupported merge table: ${table}`);const winners:AnyRecord[]=[];for(const incoming of records){if(!incoming?.id)continue;const local=await getOne<AnyRecord>(canonical,incoming.id);if(!local){winners.push(incoming);continue;}const winner=await syncMergeWinner(local,incoming);if(canonicalJson(winner)!==canonicalJson(local))winners.push(winner);}if(!winners.length)return 0;const writes={ [canonical]: winners } as MutationDeltaPayload["writes"];const mutation=await buildMutation("MEMORY_MERGE","memoryMerge",winners[0].id,deltaPayload(canonical,writes));const commitWrites:Partial<Record<DataTableName,AnyRecord[]>>={[canonical]:winners,mutations:[mutation]};await atomicPut(commitWrites);snapshot.mutations=mergeById(snapshot.mutations,[mutation]).slice(-RECENT_EVENT_LIMIT);return winners.length;}
...
export async function applySyncBootstrapChunk(memoryRoot:string,table:SyncTableName|"mutations"|string,records:AnyRecord[]){await bootBrain2();if(snapshot.memoryRoot!==memoryRoot)throw new Error("Bootstrap memory-root mismatch.");if(snapshot.storage.totalMessages>0)throw new Error("Bootstrap is only allowed into an empty Brain2 replica. Existing replicas use mutation sync.");const canonical=table==="mutations"?"mutations":canonicalIncomingSyncTable(String(table));if(!canonical)throw new Error("Unsupported bootstrap table.");await putMany(canonical,records);}
