...
type Device={id:string;space_id:string;name:string;kind:string;trusted:number;revoked:number;secret_hash:string;public_key?:string|null;last_seen_at:string;created_at:string;updated_at:string};
type Pairing={token_hash:string;space_id:string;issued_by:string;expires_at:string;consumed_at?:string|null;created_at:string};
type SignalRow={id:string;space_id:string;from_device_id:string;to_device_id:string;kind:string;payload_json:string;expires_at:string;consumed_at?:string|null;created_at:string};
...
CREATE TABLE IF NOT EXISTS pairing_tokens(token_hash TEXT PRIMARY KEY,space_id TEXT NOT NULL,issued_by TEXT NOT NULL,expires_at TEXT NOT NULL,consumed_at TEXT,created_at TEXT NOT NULL);
CREATE TABLE IF NOT EXISTS signals(id TEXT PRIMARY KEY,space_id TEXT NOT NULL,from_device_id TEXT NOT NULL,to_device_id TEXT NOT NULL,kind TEXT NOT NULL,payload_json TEXT NOT NULL,expires_at TEXT NOT NULL,consumed_at TEXT,created_at TEXT NOT NULL);
CREATE INDEX IF NOT EXISTS idx_sync_signals_recipient ON signals(to_device_id,consumed_at,created_at);`);holder.__brain2SyncDb=db;return db;}
function cleanupLocal(db:Db){const now=nowIso();db.prepare("DELETE FROM pairing_tokens WHERE expires_at<? OR consumed_at IS NOT NULL").run(now);db.prepare("DELETE FROM signals WHERE expires_at<? OR consumed_at IS NOT NULL").run(now);}

async function blobStore(){const{getStore}=await import("@netlify/blobs");return getStore({name:"brain2-sync-v4",consistency:"strong"} as any);}
const deviceKey=(deviceId:string)=>`devices/${deviceId}.json`;
const pairKey=(hash:string)=>`pairings/${hash}.json`;
const signalKey=(to:string,created:string,signalId:string)=>`signals/${to}/${created}_${signalId}.json`;
async function blobGet<T>(key:string):Promise<T|null>{const s=await blobStore();return await s.get(key,{type:"json",consistency:"strong"} as any) as T|null;}
async function blobSet<T>(key:string,value:T,options?:Record<string,unknown>){const s=await blobStore();return s.setJSON(key,value,options as any);}
async function blobList(prefix:string){const s=await blobStore();const result=await s.list({prefix,paginate:false} as any) as any;return result?.blobs||[];}
async function blobDelete(key:string){const s=await blobStore();await s.delete(key);}
...
async function registerBlob(input:{deviceId:string;spaceId?:string;name:string;kind:string;joinToken?:string;publicKey?:string}){
  let spaceId=input.spaceId?.trim()||"";let pairInfo:{pair:Pairing;tokenHash:string}|null=null;if(input.joinToken){pairInfo=await validateBlobPair(input.joinToken);spaceId=pairInfo.pair.space_id;}
  if(!spaceId)throw new Error("A Brain2 memory root or pairing token is required.");
  const existing=await blobGet<Device>(deviceKey(input.deviceId));
  if(existing){
    if(!pairInfo)throw new Error("Device already registered on this signaling server. Reuse its existing local credential or join again with a fresh pairing QR.");
    if(existing.space_id!==spaceId)throw new Error("This Brain2 device is already bound to a different memory root.");
    const deviceToken=randomBytes(32).toString("base64url"),t=nowIso();const repaired:Device={...existing,name:input.name||existing.name,kind:input.kind||existing.kind,trusted:1,revoked:0,secret_hash:sha256(deviceToken),public_key:input.publicKey||existing.public_key||null,last_seen_at:t,updated_at:t};await blobSet(deviceKey(input.deviceId),repaired);await consumeBlobPair(pairInfo.tokenHash,pairInfo.pair);return{deviceId:input.deviceId,spaceId,deviceToken,trusted:true,createdAt:existing.created_at,repaired:true};
  }
  if(!pairInfo){const devices=await blobList("devices/");let trustedCount=0;for(const item of devices){const d=await blobGet<Device>(item.key);if(d?.space_id===spaceId&&d.trusted&&!d.revoked)trustedCount++;}if(trustedCount>0)throw new Error("This Brain2 memory already has trusted devices. Join with a single-use pairing token.");}
