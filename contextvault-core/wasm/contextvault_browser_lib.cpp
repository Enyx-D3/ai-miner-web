...
std::string render_compact_topic_part(const TopicPart& part) {
    ...
    out << "## Metadata\n\n";
    out << "- Conversation items in topic part: " << part.atom_count << "\n";
...
extern "C" CvBrowserDigestResult cv_digest_chatgpt_json_to_markdown(
    ...
    files.reserve(parts.size() + 1u);

    std::ostringstream index;
    index << "# ChatGPT Export Digest\n\n";
    index << "## Overview\n\n";
    index << "- Platform: ChatGPT\n";
    index << "- Source type: ChatGPT export\n";
    index << "- Conversations or groups: " << threads.size() << "\n";
    index << "- Paragraphs: " << record.paragraphs.size() << "\n";
    index << "- Conversation Items: " << atom_index.atoms.size() << "\n";
    index << "- Topic files written: " << parts.size() << "\n";
    index << "- Max messages per file: " << max_messages_per_file << "\n\n";
    index << "## How to Use This Archive\n\n";
    index << "1. Open `index.md` to see the archive map.\n";
    index << "2. Open any file in `topics/` to read grouped conversation content.\n";
    index << "3. Use the separate `analysis_prompt.md` file with your preferred AI tool for deeper analysis.\n\n";
    index << "## Topics\n\n";
    ...
    for (const auto& part : parts) {
        ...
        index << "](" << path << ") - " << part.atom_count << " conversation items, " << part.paragraphs.size() << " messages\n";
